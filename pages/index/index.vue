					...
					<template v-if="tabBars[index1].template === 'index'">
						<block v-for="(v, i) in tab.data" :key="i">
							...
							<template v-if="v.type === 'swiper'">
								<!-- 轮播图组件 -->
								<swiper-image :resdata="v.data"></swiper-image>
							</template>
							...
							<template v-else-if="v.type === 'threeAdv'">
								<!-- 三图广告位 -->
								<three-adv :resdata="v.data"></three-adv>
								<!-- 分割线 -->
								<divider></divider>
							</template>
							...
							<template v-else-if="v.type === 'oneAdv'">
								<!-- 大图广告位 -->
								<card :headTitle="v.data.title" :bodyCover="v.data.cover"></card>
								<!-- <card :showhead="false"> 
									<block slot="title">标题</block>
									<image src="/static/images/bg.jpg" mode="width"></image>
								</card> -->
							</template>
							
							<template v-else-if="v.type === 'list'">
								<!-- 公共列表组件 750 -5 = 745 745/2= 372.5-->
								<view class="row j-sb">
